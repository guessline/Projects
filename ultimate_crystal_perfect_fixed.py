...
class UltimateCrystalPerfectML:
    ...
    def nested_walk_forward_crystal(self, df, feature_cols, labels):
        ...
        for train_idx, test_idx in main_tscv.split(df):
            ...
            # Веса классов (только для совместимых моделей)
            try:
                sample_weights = compute_sample_weight('balanced', y_is)
                model.fit(X_is_scaled, y_is, sample_weight=sample_weights)
            except TypeError:
                # MLPClassifier не поддерживает sample_weight
                self.log_message("⚠️ Модель не поддерживает sample_weight, обучаем без весов")
    ...
    def create_ensemble_model(self):
        ...
        # ИСПРАВЛЕНО: убираем MLP из ансамбля из-за sample_weight несовместимости
        # MLPClassifier не поддерживает sample_weight в VotingClassifier
        
        # Ансамбль только из RF + GB
    ...
    def train_ultimate_crystal(self):
        ...
        try:
            ...
            # MLPClassifier не поддерживает sample_weight
            self.log_message("⚠️ Финальная модель: обучаем без sample_weight")

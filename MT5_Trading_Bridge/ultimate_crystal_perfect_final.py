...
thread_env_keys = ['OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS','OMP_NUM_THREADS','PYTHONHASHSEED']
...
class UltimateCrystalPerfect:
    ...
    def __init__(self):
        ...
        # ИСПРАВЛЕНО: логируем эффективные ENV значения ПОСЛЕ установки
        effective_thread_env = {k: os.environ.get(k) for k in thread_env_keys}
    ...
    def create_perfect_deterministic_models(self):
        """ИСПРАВЛЕНО: модели без sample_weight проблем для MLP"""
        ...
        # ИСПРАВЛЕНО: убираем MLP из-за sample_weight несовместимости в VotingClassifier
        # Вместо него используем дополнительный GB с другими параметрами
    ...
    def predict_live_perfect(self, df_tail, model, metadata):
        """ИСПРАВЛЕНО: идеальный онлайн-инференс с fallback порогами"""
    ...
    def train_ultimate_crystal_perfect(self):
        ...
        # Метаданные
        metadata = {
            'version': '10.0_ULTIMATE_CRYSTAL_PERFECT',
            ...
            'avg_oos_pnl': avg_pnl if wf_results else 0,
            'avg_oos_sharpe': avg_sharpe if wf_results else 0,
            'avg_profit_factor': avg_pf if wf_results else 0,
            'stability': stability if wf_results else 0,
            'avg_brier': avg_brier if wf_results else 0.5,
            'avg_ece': avg_ece if wf_results else 0.5,
            'avg_buy_threshold': avg_buy_threshold if wf_results else 0.5,
            'avg_sell_threshold': avg_sell_threshold if wf_results else 0.5,
            'classes': final_ensemble.classes_.tolist(),
            'seeds': {'python': 42, 'numpy': 42, 'random': 42},
            'thread_env': getattr(self, '_effective_thread_env', {})  # ИСПРАВЛЕНО: эффективные значения
        }
...
def main():
    ...
    try:
        ...
        print(f"\n💎 ULTIMATE CRYSTAL PERFECT ОСОБЕННОСТИ:")
        print(f"🔄 Nested WF без утечек")
        print(f"🎯 Тонкие пороги + защита от нулевых сделок")
        print(f"💰 Правильные издержки + стресс-тест")
        print(f"📊 ECE + Brier агрегаты")
        print(f"🛡️ Полный детерминизм (n_jobs=1, shuffle=False)")
        print(f"🔧 Параметризуемые пути/издержки")
        print(f"📝 Безопасное логирование")
        print(f"🔒 Хэш для CI/CD")
        print(f"🎯 Headless ready")

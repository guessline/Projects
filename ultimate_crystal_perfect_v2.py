#!/usr/bin/env python3
"""
ULTIMATE CRYSTAL PERFECT 10/10 ML СИСТЕМА V2
...
try:
    ...
    import sklearn
    print("✅ Ultimate Crystal Perfect ML V2 готова")
...
class UltimateCrystalPerfectML:
    ...
    def __init__(self, data_file='btc_history_100k.csv', tc_pct=0.02):
        ...
        self.log_message("🏆 ULTIMATE CRYSTAL PERFECT ML V2 инициализирована")
    ...
    def pnl_with_carry_crystal(self, returns, signals, tc_pct=None):
        """ИСПРАВЛЕНО: корректный расчет PnL"""
        if tc_pct is None:
            tc_pct = self.tc_pct
        ...
        turnover = trades / len(signals) if trades > 0 else 0.0
    ...
    def optimize_dual_thresholds_perfect(self, y_proba_buy, y_proba_sell, returns, n_trials=50):
        """ИСПРАВЛЕНО: оптимизация порогов"""
        ...
        buy_range = np.linspace(0.45, 0.75, n_trials//5)
        sell_range = np.linspace(0.45, 0.75, n_trials//5)
        ...
        self.log_message(f"✅ Лучшие пороги: BUY={best_buy_thresh:.3f}, SELL={best_sell_thresh:.3f}")
    ...
    def calculate_metrics_crystal(self, returns, signals):
        ...
        return {
            ...
            'sortino': sortino,
            'max_dd': max_dd,
            'profit_factor': 0,  # Упрощено для краткости
            'win_rate': 0,
    ...
    def create_ensemble_model(self):
        """ИСПРАВЛЕНО: ансамбль без MLP (RF + GB только)"""
        ...
        # Ансамбль только RF + GB (поддерживают sample_weight)
    ...
    def nested_walk_forward_crystal(self, df, feature_cols, labels):
        ...
        main_tscv = TimeSeriesSplit(n_splits=3, test_size=None)  # Уменьшено для быстроты
        ...
        for train_idx, test_idx in main_tscv.split(df):
            fold_num += 1
            self.log_message(f"📊 WF Фолд {fold_num}/3: train={len(train_idx)}, test={len(test_idx)}")
            ...
            if y_proba_val.shape[1] == 3:
                proba_sell = y_proba_val[:, 0]
                proba_buy = y_proba_val[:, 2]
            else:
            ...
            # Логирование
            signal_counts = np.bincount(signals_test + 1, minlength=3)
    ...
    def train_ultimate_crystal(self):
        ...
        self.log_message("🏆 ULTIMATE CRYSTAL PERFECT V2 ЗАВЕРШЕНО!")
...
def main():
    ...
    try:
        ...
        print("🎉 ULTIMATE CRYSTAL PERFECT V2 успешно обучена!")
        print(f"📄 Метаданные: ultimate_crystal_metadata.json")
        print(f"🤖 Модель: ultimate_crystal_model.pkl")
